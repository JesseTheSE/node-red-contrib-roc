...
![screenshot of drivers example](./ROC_Drivers.jpg)
...
In the flow that uses the ROC drivers, you'll use the IP address of the radio (127.0.0.1 if running Node-RED from the radio connected to the ROC) as the "host" field that gets injected into the driver subflow. The "port" needs to match the terminal server port for the COM port in use.
